...
private static void registerDashboardRoutes() {
   // ===================== DASHBOARD =====================
    get("/admin/stats", (req, res) -> {
        ...
        return respond(res, ApiResponse.success(200, "Estadísticas admin (fallback)", out));
    }, GSON::toJson);
...
get("/chat/conversaciones/:id", (req, res) -> {
    ...
    try (Connection c = Database.getConnection();
         PreparedStatement ps = c.prepareStatement(sql)) {
        ps.setInt(1, id); ps.setInt(2, id); ps.setInt(3, id);
        try (ResultSet rs = ps.executeQuery()) {
            var list = new ArrayList<Map<String,Object>>();
            while (rs.next()) {
                var m = new HashMap<String,Object>();
                ...
                try { adminSoporte = rs.getObject("id_admin_soporte"); } catch (Exception ignore) {}
                m.put("id_admin_soporte", (Integer)adminSoporte);
                m.put("fecha_creacion", rs.getTimestamp("fecha_creacion"));
                Object activaObj = null;
                try { activaObj = rs.getObject("activa"); } catch (Exception ignore) {}
