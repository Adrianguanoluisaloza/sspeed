...
public final class DeliveryApi {
    ...
    static final class LoginRequest {
        @SerializedName("correo") String correo;
        @SerializedName("email")  String email;
        @SerializedName("contrasena") String contrasena;
        @SerializedName("password")   String password;
        String getCorreo()     { return correo != null ? correo : email; }
        String getContrasena() { return contrasena != null ? contrasena : password; }
    }
    static final class RegistroRequest {
        String nombre; String correo; String contrasena; String telefono;
    }
    static final class UbicacionRequest {
        Integer idUsuario; Double latitud; Double longitud;
        String direccion; String descripcion; Boolean activa;
    ...
    static final class TrackingPayload { Double latitud; Double longitud; }
    static final class EstadoUpdateRequest { String estado; }
    static final class AsignarPedidoRequest { @SerializedName("id_delivery") Integer idDelivery; }
    static final class PedidoDetallePayload {
        @SerializedName("id_producto") int idProducto;
        int cantidad; double subtotal; @SerializedName("precio_unitario") double precioUnitario;
    }
    static final class PedidoPayload {
        @SerializedName("id_cliente") int idCliente;
        @SerializedName("id_delivery") Integer idDelivery;
        @SerializedName("direccion_entrega") String direccionEntrega;
        @SerializedName("metodo_pago") String metodoPago;
        String estado; Double total; List<PedidoDetallePayload> productos;
    }
    static final class MensajePayload {
        @SerializedName("id_remitente") Integer idRemitente;
        String mensaje;
    }
   // Reemplaza tu clase interna RecomendacionPayload por esta
static final class RecomendacionPayload {
    @SerializedName("id_usuario") Integer idUsuario;
    @SerializedName("puntuacion") Integer puntuacion;
    @SerializedName("rating")     Integer rating;
    String comentario;
    Integer getPuntuacion() { return (puntuacion != null) ? puntuacion : rating; }
}
...
private static void registerDashboardRoutes() {
    get("/admin/stats", (req, res) -> {
        ...
        try (Connection c = Database.getConnection();
             PreparedStatement ps = c.prepareStatement("SELECT * FROM fn_admin_dashboard()");
            ...
            return respond(res, ApiResponse.success(200, "Estadísticas admin", out));
        } catch (Exception e) {
            res.status(500);
            return ApiResponse.error(500, "Error al consultar estadísticas del dashboard");
        }
    }, GSON::toJson);
...
private static void registerMensajeRoutes() {
    ...
    // Añade dentro de registerMensajeRoutes() o crea registerChatRoutes() si prefieres
get("/chat/conversaciones/:id", (req, res) -> {
    res.type("application/json");
    int id = parseId(req.params(":id"));
    String sql = "SELECT id_conversacion,id_pedido,id_cliente,id_delivery,id_admin_soporte,fecha_creacion,activa " +
    ...
    try (Connection c = Database.getConnection();
        ...
        try (ResultSet rs = ps.executeQuery()) {
            ...
            while (rs.next()) {
                ...
                m.put("id_admin_soporte", (Integer)rs.getObject("id_admin_soporte"));
                m.put("fecha_creacion", rs.getTimestamp("fecha_creacion"));
                m.put("activa", (Boolean)rs.getObject("activa"));
                list.add(m);
            }
            return respond(res, ApiResponse.success(200, "ok", list)); // data: [ ... ]
        }
        ...
        res.status(500);
        return ApiResponse.error(500, "Error obteniendo conversaciones");
    }
}, GSON::toJson);
...
post("/productos/:id/recomendaciones", (req, res) -> {
    ...
    // Añade dentro de registerRecomendacionRoutes(), debajo de las otras rutas
get("/recomendaciones", (req, res) -> {
    res.type("application/json");
    String sql =
        "SELECT r.id_recomendacion, r.id_producto, r.id_usuario, r.comentario, r.rating, r.fecha_creacion, " +
    ...
    try (Connection c = Database.getConnection();
        ...
        res.status(500);
        return ApiResponse.error(500, "Error listando recomendaciones");
    }
}, GSON::toJson);
...
private static void registerDeliveryRoutes() {
    path("/delivery", () -> {
        put("/:id/ubicacion", (req, res) -> {
            ...
            if (!ok) throw new ApiException(500, "No se pudo actualizar la ubicación en vivo");
    ...
    private static void registerTrackingRoutes() {
        get("/pedidos/:idPedido/tracking", (req, res) -> {
            ...
            if (ubicacion.isEmpty()) throw new ApiException(404, "No hay tracking activo para el pedido");
    ...
    private static Ubicacion toUbicacion(UbicacionRequest r) {
        if (r == null) throw new ApiException(400, "El cuerpo de la solicitud es obligatorio");
    ...
    private static int parseId(String raw) {
        ...
        catch (NumberFormatException e) { throw new ApiException(400, "Identificador inválido"); }
    }
    ...
    private static <T> T parseBody(spark.Request req, Class<T> clazz) {
        try {
            ...
            if (body == null) throw new ApiException(400, "El cuerpo de la solicitud es obligatorio");
            return body;
    ...
    private static void setupExceptionHandlers() {
        exception(ApiException.class, (ex, req, res) -> {
            res.type("application/json"); res.status(ex.getStatus());
            res.body(GSON.toJson(ApiResponse.error(ex.getStatus(), ex.getMessage(), ex.getDetails())));
        });
        exception(Exception.class, (ex, req, res) -> {
            res.type("application/json"); res.status(500);
            ex.printStackTrace();
